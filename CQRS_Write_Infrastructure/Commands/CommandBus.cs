...
namespace CQRS_Write_Infrastructure.Commands
{
    public class CommandBus : ICommandBus
    {
        ...
        public void Publish<T>(T @event) where T : IEvent
        {
            ...
            {
                throw new InvalidOperationException($"Evento não registrado: {@event}.");
            }
        }
        ...
        {
            var commandHandlerMethod = eventHandler.GetType().GetMethods()
                .Where(m => m.GetParameters()
        ...
        public void Send<T>(T command) where T : ICommand
        {
            ...
            {
                throw new InvalidOperationException($"O comando não foi registrado: {command}");
            }
        }
    }
}
